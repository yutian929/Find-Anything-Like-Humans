...
class AI2THORNode:
    def __init__(self):
        # ROS Params
        rospy.init_node("ai2thor_simulator")
        ## General
        self.bridge = CvBridge()
        ## Input/Output Topics
        self.topic_rgb = rospy.get_param("~topic_rgb", "/ai2thor/rgb")
        self.topic_depth = rospy.get_param("~topic_depth", "/ai2thor/depth")
        self.topic_camera_info = rospy.get_param(
            "~topic_camera_info", "/ai2thor/camera_info"
        )
        ## Camera/Scene Params
        ...
        self.GRID_SIZE = rospy.get_param("~grid_size", 0.25)
        ...
        # Server & Client (none for this node, but keep section for consistency)
        # ...existing code...
        ...
        self.sample_pcd_pub = rospy.Publisher(
            "/ai2thor/sample_pcd", PointCloud2, queue_size=1
        )
        rospy.Subscriber("/ithor/keyboard_cmd", String, self.keyboard_callback)
        rospy.Subscriber("/ithor/teleport_cmd", String, self.teleport_callback)
        rospy.Subscriber("/ithor/transobj_cmd", String, self.transobj_callback)

        # TF Broadcaster
        self.tf_broadcaster = tf2_ros.TransformBroadcaster()

        # Control mapping for keyboard commands
        self.controls = {
            "W": "MoveAhead",
            "A": "RotateLeft",
            "S": "MoveBack",
        ...
        rospy.loginfo("AI2THOR simulator node started")

    def renew_pose(self):
        """Renew the robot's pose"""
        ...
        self.pose["x"] = cur_position["x"]
        self.pose["y"] = -cur_position["z"]
        self.pose["z"] = cur_position["y"]
        self.pose["roll"] = 0
        self.pose["pitch"] = 0
        self.pose["yaw"] = np.deg2rad(cur_rotation["y"])

    def init_robot(self, first_event):
        """Initialize robot with the first event"""
        agent_metadata = first_event.metadata["agent"]
        self.pose["x"] = agent_metadata["position"]["x"]
        self.pose["y"] = agent_metadata["position"]["y"]
        self.pose["z"] = agent_metadata["position"]["z"]
        self.pose["roll"] = np.deg2rad(agent_metadata["rotation"]["x"])
        self.pose["pitch"] = np.deg2rad(agent_metadata["rotation"]["y"])
        self.pose["yaw"] = np.deg2rad(agent_metadata["rotation"]["z"])
    ...
    def publish_tf_frames(self):
        ...
        try:
            ...
            map2cam.transform.translation.x = self.pose["x"]
            map2cam.transform.translation.y = self.pose["y"]
            map2cam.transform.translation.z = self.pose["z"]
    ...
    def get_best_match_reachable_position(self, positions, target_position):
        """Find the best match reachable position."""
        best_match = None
    ...
    def transobj(self, name, position, rotation):
        """Transform an object to a new position and rotation."""
        try:
            self.event = self.controller.step(action="Pass")
            set_objects_poses = []
            ...
            self.event = self.controller.step(
                action="SetObjectPoses", objectPoses=set_objects_poses
            )
            self.event = self.controller.step(
                action="SetObjectPoses", objectPoses=set_objects_poses
            )
        except Exception as e:
            rospy.logerr(f"Transform object error: {str(e)}")
        ...
        try:
            ...
            if self.enable_sample_pcd:
                point_cloud = self.generate_point_cloud(
                    rgb_image, depth_image, num_points=5000
                )
                if point_cloud:
        ...
        try:
            ...
            if key in self.controls:
                ...
                if action in ["MoveAhead", "MoveBack"]:
                    self.event = self.controller.step(
                        action=action, moveMagnitude=self.GRID_SIZE
                    )
                elif action in ["RotateLeft", "RotateRight"]:
                    self.event = self.controller.step(action=action, degrees=30)
                else:
                    self.event = self.controller.step(action=action)
                self.controller.step(action="Done")
                rospy.logwarn(f"self.pose: {self.pose}")
            else:
    ...
    def teleport_callback(self, msg):
        ...
        try:
            ...
            position = cmd_dict.get("position", {"x": 0, "y": 0, "z": 0})
            rotation = cmd_dict.get("rotation", {"x": 0, "y": 0, "z": 0})
            positions = self.controller.step(action="GetReachablePositions").metadata[
                "actionReturn"
            ]
            best_match_reachable_position = self.get_best_match_reachable_position(
                positions, position
            )
            self.event = self.controller.step(
                action="Teleport",
                position=best_match_reachable_position,
                rotation=rotation,
            )
            self.controller.step(action="Done")
            rospy.logwarn(
                f"Target position: {position},\n teleporting to nearest reachable position: {best_match_reachable_position},\nrotation: {rotation}"
            )
        except Exception as e:
            rospy.logerr(f"Teleport callback error: {str(e)}")

    def transobj_callback(self, msg):
        """Handle transobj commands"""
        try:
            cmd = msg.data
            json_file = os.path.join(os.path.dirname(__file__), "transobj.json")
            current_scene_trans = json.load(open(json_file))
            current_scene_trans = current_scene_trans[self.SCENE_NAME]
            transobj_name = current_scene_trans[cmd]["name"]
            transobj_position = current_scene_trans[cmd]["position"]
            transobj_rotation = current_scene_trans[cmd]["rotation"]
            self.transobj(transobj_name, transobj_position, transobj_rotation)
        except Exception as e:
            rospy.logerr(f"Transobj callback error: {str(e)}")
    ...
    def run(self):
        ...
        rospy.loginfo("ai2thor simulator node shut down")

    def generate_point_cloud(self, rgb_image, depth_image=None, num_points=10000):
        """生成点云：如果传入深度图则生成真实3D点云，否则生成z=0的平面点云

        Args:
            rgb_image: RGB格式的彩色图像
            depth_image: 深度图像(单位:米)，如果为None则生成z=0的平面点云
            num_points: 采样的点云数量
        ...
        try:
            ...
            if depth_image is not None:
                ...
                d = depth_image[v, u].astype(np.float32)  # 深度值

                # 如果深度值单位是毫米，转换为米
                if d.max() > 100:
                    d = d / 1000.0

                # 使用convert_from_uvd函数进行坐标转换
                points3d = convert_from_uvd(u, v, d, intr, pose, depth_scale=1.0)

            else:
                # 没有深度图时，生成z=0的平面点云
                # 1. 构造像素坐标网格 (u,v)
                us = np.linspace(0, w - 1, w)
                vs = np.linspace(0, h - 1, h)
                ...
                # 如果点数太多，可以随机采样
                if len(u) > num_points:
                    idx = np.random.choice(len(u), num_points, replace=False)
                    u = u[idx]
                    v = v[idx]

                # 生成z=0的深度值
                d = np.zeros_like(u, dtype=np.float32)
...
if __name__ == "__main__":
    try:
        ai2thor_node = AI2THORNode()
        ai2thor_node.run()
    except Exception as e:
        rospy.logerr(f"AI2THOR node encountered an error: {e}")


# FloorPlan1 | CoffeeMachine_19c6af8e | -0.3, 1.1, -0.8 | 0, 0, 0
