...
def ac_compile_parallelize(
    ...
    if (
        trained_model_process_group is None
        and inference_only_models_process_groups is None
    ):
        ...
        all_pgs = [trained_model_process_group] + [None] * len(
    ...
    for m, pg in zip(all_models, all_pgs):
        ...
        for k in m.keys():
            ...
            fully_shard(
                m.backbone, **fsdp_config, reshard_after_forward=True
            ).set_reduce_scatter_divide_factor(1)
