...
def _make_dinov3_dpt_depther(
    ...
    **kwargs,
    ...
    depth_range = depth_range or _get_depth_range(
        _DPT_HEAD_CONFIG_DICT["depth_weights"]
    )
    ...
    if pretrained:
        if isinstance(depther_weights, DepthWeights):
            ...
            ), f"Unsupported depther weights {depther_weights}"
            weights_name = depther_weights.value.lower()
            hash = kwargs["hash"] if "hash" in kwargs else "02040be1"
            url = (
                DINOV3_BASE_URL
                + f"/{backbone_name}/{backbone_name}_{weights_name}_dpt_head-{hash}.pth"
            )
        else:
