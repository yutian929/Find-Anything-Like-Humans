    ...
    **kwargs,
    ...
    if backbone_name == "dinov3_vit7b16":
        ...
        raise AssertionError(
            f"No pretrained segmentation checkpoint available for {backbone_name}"
        )
    ...
    if pretrained:
        if type(segmentor_weights) is SegmentorWeights:
            assert (
                segmentor_weights == SegmentorWeights.ADE20K
            ...
            model_filename = (
                f"{backbone_name}_{segmentor_weights_name}_m2f_head-{hash}.pth"
            )
        ...
        missing_keys, unexpected_keys = segmentor.load_state_dict(
