...
class DetectorWithProcessor(torch.nn.Module):
    ...
    def forward(self, samples: list[torch.Tensor]):
        ...
        sizes_tensor = torch.tensor(
            [sample.shape[1:] for sample in samples], device=samples[0].device
        )  # N * [3, H, W]
        return self.postprocessor(
            outputs, target_sizes=sizes_tensor, original_target_sizes=sizes_tensor
        )
...
def _make_dinov3_detector(
    ...
    **kwargs,
    ...
    backbone_class = dict(
        dinov3_vit7b16=dinov3_vit7b16, dinov3_vitl16plus=dinov3_vitl16plus
    )[backbone_name]
    n_windows_sqrt = dict(dinov3_vit7b16=3, dinov3_vitl16plus=2)[backbone_name]
    backbone = backbone_class(
        pretrained=pretrained, weights=backbone_weights, check_hash=check_hash
    )
    backbone.eval()
    ...
    if pretrained:
        if (
            type(detector_weights) is DetectionWeights
            and detector_weights == DetectionWeights.COCO2017
        ):
            assert (
                detector_weights == DetectionWeights.COCO2017
            ), f"Unsupported detector weights {detector_weights}"
            detection_weights_name = detector_weights.value.lower()
            hash = kwargs["hash"] if "hash" in kwargs else "b0235ff7"
            model_filename = (
                f"{backbone_name}_{detection_weights_name}_detr_head-{hash}.pth"
            )
