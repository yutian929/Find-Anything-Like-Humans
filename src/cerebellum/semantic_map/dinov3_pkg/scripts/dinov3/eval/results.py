...
class FileFormat(Enum):
    ...
    def guess(path: Union[str, PathLike]) -> "FileFormat":
        ...
        if ext not in supported_exts:
            raise ValueError(
                f"Passed path has extension {ext}, only {list(supported_exts.keys())} are supported."
            )
...
def _assert_valid_dtypes(results: Results) -> None:
    assert _validate(
        results
    ), f"All dtypes from {results.dtypes} must be in {_VALID_DTYPES}"
...
def save_collection(
    ...
    with pd.ExcelWriter(f, engine="openpyxl", mode="w") as writer:
        for sheet_name, results in results_collection.items():
            _assert_valid_dtypes(results)
            results.to_excel(
                writer, sheet_name=sheet_name, header=True, index=False, na_rep=""
            )
