...
class WindowsWrapper(torch.nn.Module):
    ...
    def __init__(self, backbone, n_windows_w, n_windows_h, patch_size):
        ...
        self.num_channels = [
            el * 2 for el in backbone.num_channels
        ]  # resized + windows
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        window_h = (
            math.ceil((original_h // self._n_windows_h) / self._patch_size)
            * self._patch_size
        )
        window_w = (
            math.ceil((original_w // self._n_windows_w) / self._patch_size)
            * self._patch_size
        )
        all_h = [window_h] * (self._n_windows_h - 1) + [
            original_h - window_h * (self._n_windows_h - 1)
        ]
        all_w = [window_w] * (self._n_windows_w - 1) + [
            original_w - window_w * (self._n_windows_w - 1)
        ]
        all_h_cumsum = [0] + list(np.cumsum(all_h))
        all_w_cumsum = [0] + list(np.cumsum(all_w))
        window_patch_features = [
            [0 for _ in range(self._n_windows_w)] for _ in range(self._n_windows_h)
        ]
        ...
        for ih in range(self._n_windows_h):
            for iw in range(self._n_windows_w):
                ...
                window_patch_features[ih][iw] = self._backbone(
                    NestedTensor(tensors=window_tensor, mask=window_mask)
                )[0]
        ...
        concat_tensors = torch.cat(
            [
                v2.functional.resize(
                    global_features[0].tensors, size=window_tensors.shape[-2:]
                ),
                window_tensors,
            ],
            dim=1,
        ...
        global_mask = F.interpolate(
            tensor_list.mask[None].float(), size=concat_tensors.shape[-2:]
        ).to(torch.bool)[0]
