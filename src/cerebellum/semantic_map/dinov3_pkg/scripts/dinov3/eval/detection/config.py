...
class DetectionHeadConfig:
    ...
    num_queries_one2many: int = (
        1500  # was 0, number of query slots for one_to_many matching
    )
    """
    ...
    dim_feedforward: int = (
        2048  # intermediate size of the feedforward layers in the transformer blocks
    )
    ...
    backbone_use_layernorm: bool = (
        False  # whether to use layernorm on each layer of the backbone's features
    )
