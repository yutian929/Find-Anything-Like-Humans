...
class GlobalCrossAttention(nn.Module):
    ...
    def forward(
        ...
        pos_x = (
            torch.linspace(0.5, w - 0.5, w, dtype=torch.float32, device=ref_pts.device)[
                None, None, :, None
            ]
            * stride
        )  # 1, 1, w, 1
        pos_y = (
            torch.linspace(0.5, h - 0.5, h, dtype=torch.float32, device=ref_pts.device)[
                None, None, :, None
            ]
            * stride
        )  # 1, 1, h, 1
