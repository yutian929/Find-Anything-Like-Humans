    ...
    *,
    ...
    if metric_type.is_topk_accuracy_metric:
        ...
        return build_topk_accuracy_metric(
            average_type=metric_type.averaging_method, num_classes=num_classes, ks=ks
        )
    elif metric_type.is_topk_recall_metric:
        ks = ks or _make_default_ks(num_classes)
        return build_topk_recall_metric(
            average_type=metric_type.averaging_method, num_classes=num_classes, ks=ks
        )
        ...
        return GroupByAnyMatchAccuracy(
            top_k=1, num_classes=int(num_classes), dataset=dataset
        )
    elif metric_type == ClassificationMetricType.GROUPBY_ANY_MATCH_ACCURACY_5:
        return GroupByAnyMatchAccuracy(
            top_k=5, num_classes=int(num_classes), dataset=dataset
        )
        ...
        return MetricCollection(
            {"top-1": MulticlassAUROC(num_classes=int(num_classes))}
        )
    elif metric_type == ClassificationMetricType.MACRO_MULTILABEL_AVERAGE_PRECISION:
        return MetricCollection(
            {
                "top-1": MultilabelAveragePrecision(
        ...
        return MetricCollection(
            {
                "top-1": MulticlassF1Score(
                    num_classes=int(num_classes),
                    average=metric_type.averaging_method.value,
                )
            }
        )
    elif metric_type in (
        ...
        return MetricCollection(
            {"top-1": MacroAveragedMeanReciprocalRank(num_classes=int(num_classes))}
        )
    raise ValueError(f"Unknown metric type {metric_type}")


def build_topk_accuracy_metric(
    average_type: AveragingMethod, num_classes: int, ks: tuple = (1, 5)
):
...
def build_topk_recall_metric(
    average_type: AveragingMethod, num_classes: int, ks: tuple = (1, 5)
):
    metrics: Dict[str, Metric] = {
        f"top-{k}": MulticlassRecall(
            top_k=k, num_classes=int(num_classes), average=average_type.value
        )
        for k in ks
    }
...
def build_topk_any_match_accuracy_metric(num_classes: int, ks: tuple = (1, 5)):
    metrics: Dict[str, Metric] = {
        f"top-{k}": AnyMatchAccuracy(top_k=k, num_classes=int(num_classes)) for k in ks
    }
    return MetricCollection(metrics)
...
class MeanAveragePrecisionVOC2007(MultilabelPrecisionRecallCurve):
    ...
    def compute(self):
        ...
        interpolated_precisions = torch.stack(
            [
                torch.max(precision[i][recall[i] >= r])
...
class GroupByAnyMatchAccuracy(AnyMatchAccuracy):
    def __init__(
        ...
        **kwargs: Any,
        ...
        super().__init__(**kwargs)
        assert hasattr(
            dataset, "get_groupby_labels"
        ), "The dataset should have a `get_groupby_labels` method"
        self._groupby_labels: Dict[str, np.ndarray] = dataset.get_groupby_labels()
        assert hasattr(
            dataset, "get_mapped_targets"
        ), "The dataset should have a `get_mapped_targets` method"
        self._mapped_targets: torch.Tensor = torch.from_numpy(
            dataset.get_mapped_targets()
        )
    ...
    def groupby_metric(
        self, variable: np.ndarray, indices: np.ndarray, tp: torch.Tensor
    ) -> Dict[Any, Tensor]:
        groubpy_dict = {}
    ...
    def compute(self) -> Tensor:
        ...
        for label_name, label_value in self._groupby_labels.items():
            ...
            printable_results = {
                k: f"{100. * v.item():.4g}" for k, v in groupby_results.items()
            }
...
class MacroAveragedMeanReciprocalRank(Metric):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.add_state(
            "per_class_mrr",
            default=torch.zeros(self.num_classes, dtype=torch.float),
            dist_reduce_fx="sum",
        )
        self.add_state(
            "per_class_num_samples",
            default=torch.zeros(self.num_classes, dtype=torch.float),
            dist_reduce_fx="sum",
        )
    ...
    def update(self, preds: Tensor, target: torch.LongTensor) -> None:  # type: ignore
        ...
        # ranks: []
        rank_scores = 1 / (
            preds >= preds.gather(1, target[:, None].expand_as(preds))
        ).sum(dim=1)
        ...
        remaped_targets = torch.LongTensor(
            list(map(target_remap.get, target.tolist()))
        ).to(target.device)
        unique_remaped_targets, remaped_target_count = remaped_targets.unique(
            sorted=True, return_counts=True
        )
        sum_rank_scores = torch.zeros_like(
            unique_remaped_targets, dtype=torch.float
        ).scatter_add_(0, remaped_targets, rank_scores)
        unique_targets = torch.LongTensor(
            list(map(target_inv_remap.get, unique_remaped_targets.tolist()))
        ).to(target.device)
        self.per_class_mrr.index_add_(0, unique_targets, sum_rank_scores)
        self.per_class_num_samples.index_add_(
            0, unique_targets, remaped_target_count.float()
        )
