...
def compute_relative_average_scores(
    scores: Scores, inv_scores_ref: Scores = ALEXNET_INVERSE_SCORES
) -> AverageScores:
    rel_scores = {}
    for corruption_type in inv_scores_ref.keys():
        ...
        rel_scores[corruption_type] = np.mean(inv_scores_for_type) / np.mean(
            inv_scores_ref_for_type
        )
...
class ImageNet_C_Metric(Metric):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.add_state(
            "tp", torch.zeros(len(CORRUPTION_LEVEL_TO_ID)), dist_reduce_fx="sum"
        )
        self.add_state(
            "total", torch.zeros(len(CORRUPTION_LEVEL_TO_ID)), dist_reduce_fx="sum"
        )
