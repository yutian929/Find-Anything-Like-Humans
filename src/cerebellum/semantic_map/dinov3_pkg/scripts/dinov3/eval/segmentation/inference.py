...
def precompute_forward_number_for_sliding_inference(
    ...
    image_crop_nums = torch.zeros(
        dataset_len, device=distributed.get_rank(), dtype=torch.int8
    )
    ...
    for batch_img, target in test_dataloader:
        ...
        image_crop_nums[index.item()] = max(
            batch_image_crops
        )  # add information to the global tensor
...
def slide_inference(
    ...
    for h_idx in range(h_grids):
        for w_idx in range(w_grids):
            ...
            crop_pred = segmentation_model.predict(
                crop_img, rescale_to=crop_img.shape[2:]
            )
            if decoder_head_type == "m2f":
