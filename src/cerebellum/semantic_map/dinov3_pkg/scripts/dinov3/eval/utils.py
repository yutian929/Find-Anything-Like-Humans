...
def evaluate(
    ...
    for samples, (index, targets), *_ in metric_logger.log_every(
        data_loader, 10, header
    ):
        samples, targets, index = (
            samples[index >= 0],
            targets[index >= 0],
            index[index >= 0],
        )
        ...
        for k, metric in metrics.items():
            ...
            accumulators[k].update(
                preds=metric_inputs["preds"],
                target=metric_inputs["target"],
                index=index,
            )
    ...
    accumulated_results = {
        k: accumulator.accumulate() for k, accumulator in accumulators.items()
    }
...
def load_features_dict(path: str) -> Dict[str, torch.Tensor]:
    ...
    if ext == ".pt":
        ...
        features_dict = {
            key: torch.from_numpy(value) for key, value in numpy_features_dict.items()
        }
    else:
        raise ValueError(f'Unsupported features dict extension "{ext}"')
...
def average_metrics(
    eval_metrics_dict: dict[Any, dict[str, torch.Tensor]], ignore_keys: List[str] = []
):
    """
    ...
    output_metrics_dict = {}
    metrics = [
        metric for metric in eval_metrics_dict[0].keys() if metric not in ignore_keys
    ]
    for metric in metrics:
        stats_tensor = torch.tensor(
            [stat[metric] for stat in eval_metrics_dict.values()]
        )
