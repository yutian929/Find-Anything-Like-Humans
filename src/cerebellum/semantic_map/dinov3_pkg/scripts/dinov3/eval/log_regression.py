...
try:
    ...
    logger.warning(
        "Can't import scikit-learn. This is necessary for evaluating log regression"
    )
    raise ImportError
...
class TrainConfig:
    ...
    val_dataset: Optional[
        str
    ] = None  # val dataset path. If None, choose hyperparameters on 10% of the train set.
    ...
    train_features_device: str = (
        "cpu"  # device to gather train features (cpu, cuda, cuda:0, etc.)
    )
    train_dtype: str = "float64"  # data type to convert the train features to
    max_train_iters: int = (
        1_000  # maximum number of train iterations in logistic regression
    )
...
class LogRegModule(nn.Module):
    def __init__(self, C, multi_label=False, logreg_config=TrainConfig):
        ...
        ), f"SKLearn can only work on CPU device, got {self.device}"
...
def evaluate_logreg_model(
    *, logreg_model, test_metric, test_data_loader, save_results_func=None
):
...
def train_for_C(*, C, train_features, train_labels, logreg_config: TrainConfig):
    logreg_model = LogRegModule(
        C, multi_label=len(train_labels.shape) > 1, logreg_config=logreg_config
    )
...
def make_logreg_data_loader(
    batch_size: int, num_workers: int, features: torch.Tensor, labels: torch.Tensor
):
...
def make_transform(config: TransformConfig):
    if config.resize_size / config.crop_size != 1:
        logger.warning(
            f"Default resize / crop ratio is 1, here we have {config.resize_size} / {config.crop_size}"
        )
...
def make_train_val_datasets(
    train_config: TrainConfig, few_shot_config: FewShotConfig, transform
):
    ...
    if train_config.val_dataset is not None:
        ...
        train_dataset, val_dataset = split_train_val_datasets(
            train_dataset, split_percentage=split_percentage
        )
...
def make_test_dataset_and_data_loader(
    model, config: EvalConfig, transform, gather_on_cpu: bool
):
    ...
    test_data_loader = make_logreg_data_loader(
        config.batch_size, config.num_workers, test_features, test_labels
    )
    return test_dataset, test_data_loader


def eval_log_regression_with_model(
    *, model: torch.nn.Module, autocast_dtype, config: LogregEvalConfig
):
    """
    ...
    config.eval.batch_size = (
        config.eval.batch_size or config.train.batch_size
    )  # use train batch size for eval if None

    # Setting up train and val datasets
    train_dataset_dict, val_dataset, num_classes = make_train_val_datasets(
        config.train, config.few_shot, transform
    )
    ...
    with torch.autocast("cuda", dtype=autocast_dtype):
        ...
        test_dataset, test_data_loader = make_test_dataset_and_data_loader(
            model, config.eval, transform, gather_on_cpu
        )
    ...
    val_metric = build_classification_metric(
        config.train.val_metric_type, num_classes=num_classes, dataset=val_dataset
    )
    test_metric_type = config.eval.test_metric_type or config.train.val_metric_type
    test_metric = build_classification_metric(
        test_metric_type, num_classes=num_classes, dataset=test_dataset
    )
    ...
    for _try in train_data_dict.keys():
        ...
        results_dict[_try] = {
            k: v.item() * 100.0 for k, v in eval_metrics["metrics"].items()
        }
    ...
    results_string = "\n".join(
        [f"{k}: {results_dict[k]:.4g}" for k in sorted(results_dict.keys())]
    )
    logger.info("Results:\n" + results_string)
...
def benchmark_launcher(eval_args: dict[str, object]) -> dict[str, Any]:
    ...
    dataclass_config, output_dir = args_dict_to_dataclass(
        eval_args=eval_args, config_dataclass=LogregEvalConfig
    )
