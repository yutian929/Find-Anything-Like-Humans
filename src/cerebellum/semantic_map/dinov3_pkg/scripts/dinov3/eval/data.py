...
def _subset_dataset_per_class(
    ...
    for class_indices in class_indices_mapping.values():
        ...
        n_for_class = (
            max(int(len(class_indices) * n_or_percent_per_class), 1)
            if is_percent
            else n_or_percent_per_class
        )
        assert isinstance(n_for_class, int)
        filtered_index = torch.randperm(len(class_indices), generator=random_generator)[
            :n_for_class
        ]
...
def _multilabel_rebalance_subset(
    ...
    n_or_percent_per_class = n_or_percent_per_class / (
...
def split_train_val_datasets(
    train_dataset, split_percentage: float = 0.1, shuffle_train: bool = True
):
    """
    ...
    logger.info(
        f"Selecting {int(split_percentage * 100)}% of the train dataset as the validation set"
    )
    if shuffle_train:
        logger.info(
            "Shuffling train dataset before splitting in train and validation sets"
        )
    ...
    for class_indices in class_indices_mapping.values():
        ...
        n_for_val = (
            max(1, int(split_percentage * len(class_indices)))
...
def create_train_dataset_dict(
    ...
    for t in range(few_shot_n_tries):
        ...
        train_dataset_dict[t] = SubsetEx(
            train_dataset, t_subset_bool.nonzero().flatten()
        )
...
def pad_multilabel_and_collate(batch, pad_value=-1):
    ...
    padded_batch = [
        (
            ...
            (
                index,
                np.pad(targets, (0, maxlen - len(targets)), constant_values=pad_value),
            ),
        )
