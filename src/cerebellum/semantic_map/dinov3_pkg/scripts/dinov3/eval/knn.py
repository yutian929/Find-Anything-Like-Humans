...
class EvalConfig:
    ...
    batch_size: int | None = (
        None  # batch size for evaluation, None to use train batch size
    )
...
class KnnModule(torch.nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.device = device
        self.train_features_rank_T = train_features.chunk(self.world_size)[
            self.rank
        ].T.to(self.device)
        # Labels can either be integers, or in a one-hot format
        self.candidates = (
            train_labels.chunk(self.world_size)[self.rank].unsqueeze(0).to(self.device)
        )
        ...
        if self.skip_first_nn:
            logger.info(
                "Skipping the first nearest neighbor of each element in the test dataset"
            )

    def _get_knn_sims_and_labels(self, similarity, train_labels):
        topk_sims, indices = similarity.topk(
            min(self.max_k, similarity.shape[1]), largest=True, sorted=True
        )
        if len(train_labels.shape) == 3:  # If the labels are in one_hot format
            indices = indices.unsqueeze(2).expand(
    ...
    def _similarity_for_rank(self, features_rank, source_rank):
        ...
        candidate_labels = self.candidates.expand(
            len(similarity_rank), *self.candidates.shape[1:]
        )
    ...
    def compute_neighbors(self, features_rank):
        ...
        for rank in range(self.world_size):
            partial_topk_sims, partial_neighbors_labels = self._similarity_for_rank(
                features_rank, rank
            )
            gathered_topk_sims = torch.cat(gather_all_tensors(partial_topk_sims), dim=1)
            gathered_neighbor_labels = torch.cat(
                gather_all_tensors(partial_neighbors_labels), dim=1
            )
            if (
                self.rank == rank
            ):  # Performing a second top-k to get k neighbors from the gathered k * world_size
...
def make_test_data_loader(config: EvalConfig, transform):
    ...
    multilabel_collate_fn = (
        config.test_metric_type == ClassificationMetricType.ANY_MATCH_ACCURACY
    )
    ...
    )  # eval batch size has been replaced by train batch size if None

    return make_data_loader(
        dataset=DatasetWithEnumeratedTargets(
            test_dataset, pad_dataset=True, num_replicas=distributed.get_world_size()
        ),
...
def eval_knn(
    *,
    ...
    ] = {}  # {k: {try: {metric_name: metric_value}}}
    ...
    for try_ in train_data_dict.keys():
        train_features, train_labels = (
            train_data_dict[try_]["train_features"],
            train_data_dict[try_]["train_labels"],
        )
        ks = sorted(
            set(
                [
                    el if el < len(train_features) else len(train_features)
                    for el in knn_config.ks
                ]
            )
        )
        knn_module = partial_knn_module(
            train_features=train_features, train_labels=train_labels, ks=ks
        )
        postprocessors, metrics = {k: DictKeysModule([k]) for k in ks}, {
            k: metric_collection.clone() for k in ks
        }
        ...
        for k in ks:
            if save_results:
                if len(train_data_dict) > 1:
                    split_results_saver = partial(
                        save_results_func, filename_suffix=f"try_{try_}_k_{k}"
                    )
                else:
                    split_results_saver = partial(
                        save_results_func, filename_suffix=f"k_{k}"
                    )
                split_results_saver(**accumulated_results[k])

            if k not in eval_metrics_dict:
                eval_metrics_dict[k] = {}
            eval_metrics_dict[k][try_] = {
                metric: v.item() * 100.0 for metric, v in eval_metrics[k].items()
            }

    if len(train_data_dict) > 1:
        return {
            k: average_metrics(eval_metrics_dict[k]) for k in eval_metrics_dict.keys()
        }

    return {k: eval_metrics_dict[k][0] for k in eval_metrics_dict.keys()}


def _log_and_format_results_dict(
    input_results_dict, few_shot_n_tries: int
) -> Dict[str, float]:
    results_dict = {}
    for knn_ in input_results_dict.keys():
        if few_shot_n_tries == 1:
            ...
            top1_mean, top1_std = (
                input_results_dict[knn_]["top-1_mean"],
                input_results_dict[knn_]["top-1_std"],
            )
            ...
            if "top-5_mean" in input_results_dict[knn_]:
                top5_mean, top5_std = (
                    input_results_dict[knn_]["top-5_mean"],
                    input_results_dict[knn_]["top-5_std"],
                )
...
def eval_knn_with_model(
    *, model: torch.nn.Module, autocast_dtype, config: KnnEvalConfig
):
    start = time.time()
    ...
    metric_collection = build_classification_metric(
        config.eval.test_metric_type, num_classes=num_classes
    )
    ...
    if config.save_results:
        save_results_func = partial(
            default_save_results_func, output_dir=config.output_dir
        )
    ...
    results_dict = _log_and_format_results_dict(
        results_dict_knn, config.few_shot.n_tries
    )
...
def benchmark_launcher(eval_args: dict[str, object]) -> dict[str, Any]:
    ...
    dataclass_config, output_dir = args_dict_to_dataclass(
        eval_args=eval_args, config_dataclass=KnnEvalConfig
    )
    model, model_context = load_model_and_context(
        dataclass_config.model, output_dir=output_dir
    )
