    ...
    def get_optimizer(self, optim_param_groups):
        if self == OptimizerType.ADAMW:
            ...
            optimizer = torch.optim.SGD(
                optim_param_groups, momentum=0.9, weight_decay=0
            )
        return optimizer
...
class SchedulerType(Enum):
    ...
    def get_scheduler(
        self, optimizer, optim_param_groups, epoch_length, epochs, max_iter
    ):
        if self == SchedulerType.ONE_CYCLE:
            lr_list = [
                optim_param_groups[i]["lr"] for i in range(len(optim_param_groups))
            ]
...
class TrainConfig:
    ...
    learning_rates: Tuple[
        float, ...
    ] = _DEFAULT_LR_LIST  # learning rates to grid search
    n_last_blocks_list: Tuple[int] = (
        1,
    )  # number of backbone last blocks used for the linear classifier
    ...
    eval_period_iterations: int | None = (
        None  # number of iterations between two evaluations (default: one epoch)
    )
    checkpoint_retention_policy: CheckpointRetentionPolicy = (
        CheckpointRetentionPolicy.NONE
    )  # keep checkpoints or not
    resume: bool = True  # whether to resume from existing checkpoints
    classifier_fpath: Optional[
        str
    ] = None  # path to a file containing pretrained linear classifiers
    ...
    def evaluate_and_maybe_save(
        ...
        if self.save_results_func is not None:
            self.save_results_func(
                filename_suffix=f"{self.dataset_str}{save_filename_suffix}",
                **accumulated_best_results,
            )

        results_dict = {
            self.main_metric_name: 100.0
            * full_results_dict["best_classifier"]["accuracy"],
...
def setup_linear_training(
    *,
    ...
    optimizer = config.optimizer_type.get_optimizer(
    ...
    if config.resume and (
        last_checkpoint_dir := find_latest_checkpoint(
            config.classifier_fpath or checkpoint_output_dir
        )
    ):
...
def train_linear_classifiers(
    *,
    ...
    checkpoint_period = (
        train_config.save_checkpoint_iterations or train_config.epoch_length
    )
    ...
    for data, labels in metric_logger.log_every(
        ...
        is_ckpt_iteration = (
            (iteration + 1) % checkpoint_period == 0
        ...
        if is_ckpt_iteration:
            ...
            if distributed.is_subgroup_main_process():
                ...
                keep_last_n_checkpoints(
                    ckpt_dir, train_config.checkpoint_retention_policy.max_to_keep
                )
...
def eval_linear_with_model(
    *, model: torch.nn.Module, autocast_dtype, config: LinearEvalConfig
):
    start = time.time()
    ...
    for _try in train_dataset_dict.keys():
        if len(train_dataset_dict) > 1:
            checkpoint_output_dir = os.path.join(
                config.output_dir, f"checkpoints_{_try}"
            )
    ...
    for checkpoint_output_dir in checkpoint_output_dirs:
        if distributed.is_subgroup_main_process():
            cleanup_checkpoint(
                checkpoint_output_dir, config.train.checkpoint_retention_policy
            )
...
def benchmark_launcher(eval_args: dict[str, object]) -> dict[str, Any]:
    ...
    dataclass_config, output_dir = args_dict_to_dataclass(
        eval_args=eval_args, config_dataclass=LinearEvalConfig
    )
