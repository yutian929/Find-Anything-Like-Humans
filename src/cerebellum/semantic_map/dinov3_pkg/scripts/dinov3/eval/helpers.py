...
def args_dict_to_dataclass(
    eval_args: dict[str, object], config_dataclass, save_config: bool = True
) -> tuple[Any, str]:
    """
    ...
    if CONFIG_FILE_KEY in eval_args:
        ...
        eval_args_config = OmegaConf.merge(
            OmegaConf.load(config_file), OmegaConf.create(eval_args)
        )
    else:
        eval_args_config = OmegaConf.create(eval_args)

    structured_config = OmegaConf.merge(
        OmegaConf.structured(config_dataclass), eval_args_config
    )
    ...
    if save_config and dinov3.distributed.is_main_process():
        OmegaConf.save(
            config=structured_config, f=os.path.join(output_dir, EVAL_CONFIG_FNAME)
        )
