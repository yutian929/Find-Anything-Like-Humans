...
def _parse_slurm_node_list(s: str) -> List[str]:
    return subprocess.check_output(
        ["scontrol", "show", "hostnames", s], text=True
    ).splitlines()
        ...
        if not overwrite:
            for k, v in env_vars.items():
                ...
                if os.environ[k] == v:
                    continue
                raise RuntimeError(
                    f"Cannot export environment variables as {k} is already set"
                )
...
def new_subgroups(all_subgroup_ranks: Sequence[Sequence[int]]):
    ...
    """
    all_ranks = tuple(
        rank for subgroup_ranks in all_subgroup_ranks for rank in subgroup_ranks
    )
    rank = get_rank()
