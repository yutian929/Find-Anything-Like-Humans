...
def _enumerate_all_subgroup_ranks(all_subgroup_rank_spans: Sequence[Tuple[int, int]]):
    ...
    for first, last in all_subgroup_rank_spans:
        assert first <= last
    return tuple(
        tuple(range(first, last + 1)) for first, last in all_subgroup_rank_spans
    )
