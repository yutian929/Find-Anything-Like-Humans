...
class SSLMetaArch(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.dino.koleo_loss_distributed:
            ...
            assert (
                cfg.dino.koleo_topk == 1
            ), "Non-distributed KoLeo loss only supports `dino.koleo_topk=1`"
        ...
        assert (
            0 <= cfg.ibot.mask_sample_probability <= 1
        ...
        if self.gram_use_loss:
            ...
            self.gram_ema_teacher = (
                self.cfg.gram.ema_teacher
            )  # If true use the EMA_teacher as gram_teacher
            self.gram_ckpt = (
                self.cfg.gram.ckpt
            )  # Checkpoint to the first gram teacher model
            self.gram_img_level = (
                self.cfg.gram.img_level
            )  # Apply the loss on the image, if false on the batch
            self.gram_tokens_used = (
                self.cfg.gram.tokens_used
            )  # Any value in ["all", "masked", "unmasked"]
            # Update the teacher frequently
            self.gram_rep_update = (
                self.cfg.gram.rep_update
            )  # bool, if yes the gram teacher will be updated at the freq
            self.gram_update_frequency = (
                self.cfg.gram.update_frequency
            )  # defined by this var update_frequency
            self.gram_it_first_update = (
                self.cfg.gram.it_first_update
            )  # after iteration it_first_update is passed.
            ...
            self.gram_compute_stats = (
                self.cfg.gram.compute_stats
            )  # whether to compute auxiliary stats
            ...
            self.gram_global_teacher_resize_method = (
                cfg.gram.global_teacher_resize_method
            )
            self.gram_global_teacher_resize_antialias = (
                cfg.gram.global_teacher_resize_antialias
            )
            logger.info(
    ...
    def broadcast_to_subgroups(self, tensor, over_dim, global_batch_size=None):
        ...
        return catted.chunk(subgroup_size, dim=over_dim)[
            distributed.get_subgroup_rank()
        ].clone()
