...
For python-based LazyConfig, use "path.key=value".
    ...
    parser.add_argument(
        "--dump-fsdp-weights", action="store_true", help="dump fsdp weights"
    )
    parser.add_argument(
        "--record_ref_losses", action="store_true", help="record reference losses"
    )
    parser.add_argument("--ref_losses_path", default="", type=str)
    parser.add_argument(
        "--multi-distillation", action="store_true", help="run multi-distillation"
    )
...
def build_schedulers_v2(cfg):
    ...
    if cfg.optim.scaling_rule == "linear_wrt_256":
        ...
        lr_peak *= 4 * math.sqrt(
            cfg.train.batch_size_per_gpu * distributed.get_world_size() / 1024.0
        )
        lr_end *= 4 * math.sqrt(
            cfg.train.batch_size_per_gpu * distributed.get_world_size() / 1024.0
        )
...
def build_data_loader_from_cfg(
    ...
    if cfg.multidistillation.enabled:
        assert (
            cfg.multidistillation.global_batch_size % distributed.get_subgroup_size()
            == 0
        )
        local_batch_size = (
            cfg.multidistillation.global_batch_size // distributed.get_subgroup_size()
...
def build_multi_resolution_data_loader_from_cfg(
    ...
    gram_teacher_crops_sizes = (
        ...
        if cfg.crops.gram_teacher_crops_size is None
        or isinstance(cfg.crops.gram_teacher_crops_size, int)
    ...
    assert (
        len(global_crops_sizes)
        == len(local_crops_sizes)
        == len(gram_teacher_crops_sizes)
        == len(loader_ratios)
    )
    ...
    for increment, (
        ...
        loaders.append(
            build_data_loader_from_cfg(cfg=cfg_i, model=model, start_iter=start_iter)
        )
...
def do_train(cfg, model, resume=False):
    ...
    if cfg.multidistillation.enabled:
        register_dont_save_hooks(
            model,
            dont_save=[
                k for k, _ in model.state_dict().items() if k.startswith("teacher")
            ],
        )
    model.init_weights()
    start_iter = 0
    ...
    if (
        ...
        num_gram_updates = math.ceil(
            (start_iter + 1 - cfg.gram.it_first_update) / cfg.gram.update_frequency
        )
        logger.info(
            f"Gram was updated {num_gram_updates} times before iteration {start_iter}"
        )
    ...
    for data in metric_logger.log_every(
        ...
        metrics_values = torch.stack(
            [
                torch.as_tensor(
                    v, dtype=torch.float32, device=total_loss.device
                ).detach()
        ...
        if (
            ...
            and (
                cfg.gram.max_updates is None or num_gram_updates < cfg.gram.max_updates
            )
        ):
        ...
        if (iteration + 1) % cfg.checkpointing.period == 0:
            ...
            if distributed.is_subgroup_main_process():
                ...
                if (
                    "keep_every" in cfg.checkpointing
                    and (iteration + 1) % cfg.checkpointing.keep_every == 0
                ):
...
def main(argv=None):
    ...
    if meta_arch is None:
        raise ValueError(
            f"Unknown MODEL.META_ARCHITECTURE {cfg.MODEL.META_ARCHITECTURE}"
        )
