...
class MultiDistillationMetaArch(SSLMetaArch):
    ...
    def get_teacher_output(
        ...
        *,
        ...
        masked_patch_after_head = self.teacher.ibot_head(
            ibot_patch.flatten(0, 1), no_last_layer=True
        )
        masked_patch_after_head = masked_patch_after_head.view(
            n_crops, -1, *masked_patch_after_head.shape[1:]
        )
        ...
        buffer = torch.index_select(
            masked_patch_after_head.flatten(0, 1), dim=0, index=mask_indices_list
        )
        masked_patch_after_head = self.teacher.ibot_head(buffer, only_last_layer=True)

        # DINO head on CLS tokens
        cls_after_head = self.teacher.dino_head(
            cls, no_last_layer=True
        )  # [n_crops * B, K]
        cls_after_head = cls_after_head.view(n_crops, -1, *cls_after_head.shape[1:])
        cls_after_head = self.broadcast_to_subgroups(
            cls_after_head, over_dim=1, global_batch_size=global_batch_size
        )
        B = cls_after_head.shape[1]
        cls_after_head = cls_after_head.flatten(0, 1)
        cls_after_head = self.teacher.dino_head(
            cls_after_head, only_last_layer=True
        )  # [n_crops * B, K]
        # End of multidistillation codepath
