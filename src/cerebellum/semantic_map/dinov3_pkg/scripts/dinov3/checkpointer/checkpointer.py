...
def save_checkpoint(
    ...
    **others: Stateful,
    ...
    torch.distributed.broadcast_object_list(
        ckpt_dir_exists, src=src_rank, group=process_group
    )
    ...
    ckpt_dir_tmp = [
        tempfile.mkdtemp(dir=ckpt_dir.parent, prefix=ckpt_dir.name)
        if rank == 0
        else None
    ]
    torch.distributed.broadcast_object_list(
        ckpt_dir_tmp, src=src_rank, group=process_group
    )
    ...
    **others: Stateful,
    ...
    dcp.load(
        ...
        planner=dcp.default_planner.DefaultLoadPlanner(
            allow_partial_load=not strict_loading
        ),
        process_group=process_group,
    )
...
def register_dont_save_hooks(module: torch.nn.Module, dont_save: Sequence[str]):
    ...
    def load_state_dict_post_hook(module, incompatible_keys):
        ...
        for missing_key in incompatible_keys.missing_keys:
            ...
            )  # Added by activation checkpointing
            if k in _dont_save:
    ...
    _dont_save = set(
        name.replace("_checkpoint_wrapped_module.", "") for name in dont_save
    )
    _prefix = None
    ...
    if not Path(checkpoint_path).is_dir():  # PyTorch standard checkpoint
        ...
        chkpt = {
            key: (
                ...
                if not any(
                    key_not_sharded in key for key_not_sharded in keys_not_sharded
                )
                else tensor
            )
        ...
        load_checkpoint(
            ckpt_dir=checkpoint_path, model=model, process_group=process_group
        )
