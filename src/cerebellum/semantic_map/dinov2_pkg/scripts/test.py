import rospy
from dinov2_pkg.srv import DINOv2, DINOv2Request
...
def test_dinov2_service():
    rospy.init_node("test_dinov2_node", anonymous=True)
    rospy.wait_for_service("dinov2")
    dinov2_service = rospy.ServiceProxy("dinov2", DINOv2)
    ...
    # Test encode_text
    try:
        text_request = DINOv2Request()
        text_request.mode = "encode_text"
        text_request.text = "A photo of a dog"
        text_response = dinov2_service(text_request)
        text_feature = text_response.dinov2_fts
        rospy.loginfo(
            f"Text encoding result: {text_feature[:10]}... (dim: {text_response.dinov2_ft_dim})"
        )
    except rospy.ServiceException as e:
        rospy.logerr(f"Failed to call dinov2 service for encode_text: {e}")
        return
    ...
    try:
        ...
        image_request = DINOv2Request()
        image_request.mode = "encode_image"
        image_request.image = ros_image
        image_response = dinov2_service(image_request)
        cat_image_feature = image_response.dinov2_fts
        rospy.loginfo(
            f"Cat image encoding result: {cat_image_feature[:10]}... (dim: {image_response.dinov2_ft_dim})"
        )
        ...
        rospy.logerr(f"Failed to call dinov2 service for encode_image: {e}")
    ...
    try:
        ...
        images_request = DINOv2Request()
        images_request.mode = "encode_images"
        images_request.images = ros_images
        images_response = dinov2_service(images_request)
        image_features = np.array(images_response.dinov2_fts).reshape(
            -1, images_response.dinov2_ft_dim
        )
        rospy.loginfo(
            f"Images encoding result: {image_features[:10]}... (dim: {images_response.dinov2_ft_dim})"
        )
        ...
        rospy.logerr(f"Failed to call dinov2 service for encode_images: {e}")
