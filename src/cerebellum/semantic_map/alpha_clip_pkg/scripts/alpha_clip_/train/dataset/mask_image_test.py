...
class COCO_Masked_Test(Dataset):
    def __init__(
        ...
        masked_color=[255, 255, 255],
        root_directory="data/coco/val2017",
        hi_res=False,
    ):
    ...
    def __getitem__(self, index):
        ...
        image = np.array(
            Image.open(
                os.path.join(self.image_directory, self.coco.imgs[img_id]["file_name"])
            ).convert("RGB")
        )
        bbox_xywh = np.copy(np.array(ann["bbox"]))
        binary_mask = self.coco.annToMask(ann)
        cropped_image, cropped_mask = masked_crop(
            image,
            bbox_xywh,
            binary_mask,
            crop_scale=self.crop_scale,
            masked_color=self.masked_color,
        )
...
class LVIS_Masked_Test(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = np.array(
            Image.open(
                self.lvis.imgs[img_id]["coco_url"].replace(
                    "http://images.cocodataset.org", "data/coco"
                )
            ).convert("RGB")
        )
