...
class Imagenet_S(Dataset):
    def __init__(
        self, ann_file="data/imagenet_s/imagenet_919.json", hi_res=False, all_one=False
    ):
    ...
    def __getitem__(self, index):
        ...
        if self.choice == "padding":
            if max(h, w) == w:
                ...
                l, r = pad, w - h - pad
                rgba = np.pad(
                    rgba, ((l, r), (0, 0), (0, 0)), "constant", constant_values=0
                )
            else:
                pad = (h - w) // 2
                l, r = pad, h - w - pad
                rgba = np.pad(
                    rgba, ((0, 0), (l, r), (0, 0)), "constant", constant_values=0
                )
        else:
            if min(h, w) == h:
