...
def crop(image: np.array, bbox_xywh: np.array, bi_mask: np.array, scale=1.5):
    ...
    w_r = (
        int(tl_x + (1 + half_scale) * r)
        if (tl_x + (1 + half_scale) * r) < image_w
        else image_w - 1
    )
    h_t = int(tl_y - half_scale * r) if (tl_y - half_scale * r) > 0 else 0
    h_b = (
        int(tl_y + (1 + half_scale) * r)
        if (tl_y + (1 + half_scale) * r) < image_h
        else image_h - 1
    )
...
def masked_crop(
    ...
    image = np.pad(
        image, ((600, 600), (600, 600), (0, 0)), "constant", constant_values=255
    )
...
class ImageNet_Masked(Dataset):
    def __init__(
        self, ann_file="M_ImageNet_top_460k.json", masked_color=[255, 255, 255]
    ):
    ...
    def __getitem__(self, index):
        ...
        text = (
            text.replace(".", f", probably {synonyms[0]}")
            .replace(" ", "_")
            .replace("/", "_")
            .replace("\\", "_")
        )
        image[np.nonzero(binary_mask == 1)] = (
            0.5 * image[np.nonzero(binary_mask == 1)] + 0.5 * np.array([0, 255, 0])
        ).astype(np.uint8)
        os.makedirs(
            os.path.split(img_pth.replace("imagenet-21k/images", "visual_train_c"))[0],
            exist_ok=True,
        )
        Image.fromarray(image).save(
            os.path.split(img_pth.replace("imagenet-21k/images", "visual_train_c"))[0]
            + f"/{text}_"
            + os.path.split(img_pth.replace("imagenet-21k/images", "visual_train_c"))[1]
        )
