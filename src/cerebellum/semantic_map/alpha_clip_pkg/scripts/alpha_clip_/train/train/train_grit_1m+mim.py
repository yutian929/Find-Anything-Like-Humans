    ...
    def __init__(
        ...
        if lora_rank == -1:
            self.model, _ = alpha_clip.load(
                "ViT-B/16", device="cpu", lora_adapt=False, rank=-1
            )
        else:
            self.model, _ = alpha_clip.load(
                "ViT-B/16", device="cpu", lora_adapt=True, rank=lora_rank
            )
