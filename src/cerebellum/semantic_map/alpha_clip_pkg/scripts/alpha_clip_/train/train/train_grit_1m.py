...
class CLIP_Clean_Train:
    def __init__(
        ...
        if lora_rank == -1:
            self.model, _ = alpha_clip.load(
                "ViT-L/14@336px", device="cpu", lora_adapt=False, rank=-1
            )
        else:
            self.model, _ = alpha_clip.load(
                "ViT-L/14", device="cpu", lora_adapt=True, rank=lora_rank
            )
    ...
    def train_epoch(self, dataloader, test_loaders, epoch, start_iter=0, amp=False):
        ...
        for i, (images, masks, texts) in enumerate(
            tqdm(dataloader, disable=(dist.get_rank() != 0))
        ):
            ...
            if step % 100 == 0:
                if dist.get_rank() == 0:
                    self.writer.add_scalar(
                        "hyper/lr", self.optimizer.param_groups[0]["lr"], step
                    )
                    self.writer.add_scalar(
                        "logit_scale/train", self.model.logit_scale.item(), step
                    )
                    print("=====================================")
                    print(
                        f"train lr (alpha conv) step {step}: {self.optimizer.param_groups[0]['lr']}"
                    )
                    print(
                        f"train lr (other layer) step {step}: {self.optimizer.param_groups[1]['lr']}"
                    )
                    print(
    ...
    def test_epoch(self, dataloader):
        temp_corr_dict = dict()
        for i, (images, masks, target) in enumerate(
            tqdm(dataloader, disable=(dist.get_rank() != 0))
        ):
            images = images.cuda()
            target = target.cuda()
    ...
    def test(self, epoch=0):
        ...
        testset = Imagenet_S()
        self.text_embeddings = zeroshot_classifier(
            testset.classes, simple_templates, self.model, self.local_rank
        )
    ...
    def train(self, common_pair=False, resume=False, amp=False, warmup_length=200):
        ...
        for name, testset in zip(
            ["COCO", "Imagenet-S", "Imagenet-S_all_one"],
            [testset_coco, testset_image_s, testset_image_s_all_one],
        ):
        ...
        if resume == True and os.listdir(self.ckptdir) != []:
            ...
            self.model.visual.load_state_dict(
                torch.load(
                    os.path.join(self.ckptdir, resume_pth), map_location=map_location
                )
            )
        ...
        for epoch in range(start_epoch, self.num_epoch):
            # train
            if (
                trainset.__len__() * epoch
            ) > 4000 * self.batch_size * 256:  # for 64 only
                print(trainset.__len__(), epoch)
                break
            loss = self.train_epoch(
                train_loader, test_loaders, epoch, start_iter=resume_iter, amp=amp
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lora_rank", default=-1, type=int, help="lora rank (-1 to not use lora)."
    )
    parser.add_argument(
        "--common_pair",
        default=0.0,
        type=float,
        help="propotion to use image with all 1 alpha and whole caption.",
    )
    parser.add_argument(
        "--para_gamma", default=0.01, type=float, help="para_gamma of other parameters"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--exp_name", default="auto", type=str, help="specify experiment name."
    )
