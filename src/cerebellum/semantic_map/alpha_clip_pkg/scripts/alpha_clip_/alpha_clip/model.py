...
class Attention(nn.Module):
    def __init__(
        ...
        if lora_adapt:
            ...
            self.in_proj = lora.MergedLinear(
                dim, 3 * dim, r=rank, enable_lora=[True, False, True]
            )
        else:
        ...
        self.out_proj = (
            nn.Linear(dim, dim) if not lora_adapt else lora.Linear(dim, dim, r=rank)
        )
            ...
            OrderedDict(
                [
                    (
                        ...
                        if not lora_adapt
                        else lora.Linear(d_model, d_model * 4, r=rank),
                    ),
                    ("gelu", QuickGELU()),
                    (
                        "c_proj",
                        ...
                        if not lora_adapt
                        else lora.Linear(d_model * 4, d_model, r=rank),
                    ),
                ]
            )
        )
        ...
        self.resblocks = nn.Sequential(
            *[
                CustomResidualAttentionBlock(
                    width, heads, attn_mask, lora_adapt=lora_adapt, rank=rank
                )
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        self.transformer = CustomTransformer(
            width, layers, heads, lora_adapt=lora_adapt, rank=rank
        )
...
def build_model(state_dict: dict, lora_adapt=False, rank=16):
    ...
    if (
        "visual.conv1_alpha.weight" not in state_dict.keys()
    ):  # zero initialization on alpha channel
