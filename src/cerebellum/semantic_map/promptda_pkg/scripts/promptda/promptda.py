...
class PromptDA(nn.Module):
    ...
    def __init__(self, encoder="vits", ckpt_path="checkpoints/promptda_vits.ckpt"):
        super().__init__()
        ...
        module_path = Path(
            __file__
        )  # From anywhere else: module_path = Path(inspect.getfile(PromptDA))
        package_base_dir = str(
            Path(*module_path.parts[:-2])
        )  # extract path to PromptDA module, then resolve to repo base dir for dinov2 load
