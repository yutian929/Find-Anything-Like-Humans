...
def run_evit_model_pytorch(img, bboxes, model, timer=None):
    ...
    masks, iou_preds = model.predict(
        boxes=boxes, multimask_output=True, return_logits=False, timer=timer
    )
...
def run_evit_model_onnx(img, bboxes, model, timer=None):
    ...
    masks, iou_preds = model.predict_torch(
        im_size=origin_image_size, boxes=boxes, timer=timer
    )
    return masks, iou_preds
...
def run_evit_model_tensorrt(img, bboxes, model):
    ...
    origin_image_size = img.shape[:2]
    input_size = EvitResize.get_preprocess_shape(
        *origin_image_size, long_side_length=1024
    )
