...
class Model:
    ...
    def __init__(self, name, checkpoints_path=None):
        ...
        if name == "mdef_detr":
            ...
            print(
                f"Please provide correct models to use in configuration. "
...
def _make_backbone(backbone_name="resnet101", mask: bool = True):
    backbone = Backbone(
        backbone_name, train_backbone=True, return_interm_layers=mask, dilation=False
    )
...
def _make_mdef_detr(checkpoints_path):
    ...
    transformer = MDefDETRTransformer(
        d_model=256,
...
def _make_mdef_detr_minus_language(checkpoints_path):
    backbone = _make_backbone()
    transformer = MDefDETRMinusLanguageTransformer(
        d_model=256,
        return_intermediate_dec=True,
        num_feature_levels=4,
        dim_feedforward=1024,
    )
    model = MDefDETRMinusLanguage(
        backbone=backbone,
        transformer=transformer,
        num_classes=1,
        num_queries=300,
