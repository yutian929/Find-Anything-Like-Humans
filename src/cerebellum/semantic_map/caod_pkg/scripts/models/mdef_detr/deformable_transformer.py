...
class DeformableTransformer(nn.Module):
    def __init__(
        ...
        img_text_layer = ImageTextTransformerLayer(
            d_model, 8, dim_feedforward, dropout, activation
        )
    ...
    def forward(
        ...
        hs_mask = torch.zeros(
            (hs.shape[0], hs.shape[1]), dtype=torch.bool, device=hs.device
        )
        ...
        return (
            torch.stack(hs),
            init_reference_out,
            torch.stack(inter_references_out),
            torch.stack(text_memory_dec),
        )
