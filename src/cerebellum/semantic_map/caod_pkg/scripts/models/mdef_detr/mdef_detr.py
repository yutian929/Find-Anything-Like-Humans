    ...
    def __init__(
        ...
        if with_box_refine:
            ...
            self.class_embed = nn.ModuleList(
                [self.class_embed for _ in range(transformer.img_text_attn.num_layers)]
            )
            self.bbox_embed = nn.ModuleList(
                [self.bbox_embed for _ in range(transformer.img_text_attn.num_layers)]
            )
        ...
        if encode_and_save:
            ...
            if self.contrastive_align_loss:
                ...
                proj_tokens = F.normalize(
                    self.contrastive_align_projection_text(text_memory).transpose(1, 2),
                    p=2,
                    dim=-1,
                )
                out.update(
