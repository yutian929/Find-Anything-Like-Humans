...
class ModulatedDetection(Inference):
    ...
    def infer_image_multi_crop(self, image_path, **kwargs):
        ...
        memory_cache = self.model(
            imgs, [caption for i in range(imgs.shape[0])], encode_and_save=True
        )
        outputs = self.model(
            imgs, [caption], encode_and_save=False, memory_cache=memory_cache
        )
        all_boxes = []
        all_scores = []
        for i in range(len(crops)):
            ...
            # convert boxes from [0; 1] to image scales
            bboxes_scaled = self.rescale_bboxes(
                outputs["pred_boxes"].cpu()[i, keep], im.size
            )
            kept_probs = probas[keep]
            # Convert outputs to the required format
            bboxes = list(bboxes_scaled.numpy())
