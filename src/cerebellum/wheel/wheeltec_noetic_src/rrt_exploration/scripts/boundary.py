...
def node():
    ...
    # define the rosparam
    mapFrame = rospy.get_param(
        "~map_frame", "map"
    )  # the name frame boundary going to be published on
    n_point = rospy.get_param(
        "~n_point", 4
    )  # the number of the boundary point going to be created
    topicInput = rospy.get_param(
        "~topicInput", "/clicked_point"
    )  # the number of the boundary point going to be created
    topicOutput = rospy.get_param(
        "~topicOutput", "/exploration_boundary"
    )  # specify the topic which the node is going to published into the list
    frequency = rospy.get_param(
        "~frequency", 1.0
    )  # number of the rate listening to the ros param
    timeInterval = rospy.get_param(
        "~timeInterval", 5.0
    )  # the name frame boundary going to be published on
    ...
    rospy.loginfo(
        "boundary polygon: received all " + str(n_point) + " points from the user"
    )
    ...
    while not rospy.is_shutdown():
        ...
        if nextTimeReport <= rospy.get_rostime().secs:
            rospy.loginfo(
                ...
                % (rospy.get_rostime().secs, rospy.get_rostime().secs - start_time)
            )
