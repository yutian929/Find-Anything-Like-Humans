...
Input::Input(ros::NodeHandle private_nh, uint16_t port)
  ...
  private_nh.param("device_ip_difop", devip_str_difop,
                   std::string("192.168.1.102"));
...
InputSocket::InputSocket(ros::NodeHandle private_nh, uint16_t port)
  ...
  if (add_multicast) {
    ...
    if (setsockopt(sockfd_, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char *)&group,
                   sizeof(group)) < 0) {
...
void Input::UDP_difop() {
  ...
  for (int k = 0; k < 10; k++) {
    ...
    data[186] = 0xFA;
    data[187] = 0xFB;
    int rtn = sendto(sockfd_, data, 188, 0, (struct sockaddr *)&server_sai,
...
void Input::UDP_order(const std_msgs::Int8 msg) {
  ...
  for (int k = 0; k < 10; k++) {
    ...
    rtn = sendto(sockfd_, data, 188, 0, (struct sockaddr *)&server_sai,
                 sizeof(struct sockaddr));
    if (rtn < 0) {
      printf("start scan error !\n");
    } else {
