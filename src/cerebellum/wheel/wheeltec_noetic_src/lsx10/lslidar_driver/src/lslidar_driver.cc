...
{
  pubscan_thread_ = new boost::thread(
      boost::bind(&LslidarDriver::pubScanThread, this)); // 创建新线程
  difop_switch = nh.subscribe<std_msgs::Int8>(
      "lslidar_order", 1, &LslidarDriver::lidar_order, this); // 指令输入
  is_start = true;
  ...
  pnh.param<std::string>("pointcloud_topic", pointcloud_topic,
                         "lslidar_point_cloud");
...
void LslidarDriver::lidar_order(const std_msgs::Int8 msg) // 发送控制雷达指令
{
  ...
  if (interface_selection == "net")
    ...
    for (int k = 0; k < 10; k++) {
      ...
      if (lidar_name == "M10" || lidar_name == "M10_GPS" ||
          lidar_name == "M10_P" || lidar_name == "M10_DOUBLE") {
        if (i <= 1) // 控制雷达启停
        {
          data[184] = 0x01;
          data[185] = char(i);
...
bool LslidarDriver::createRosIO() // 网口初始化
{
  ...
  if (dump_file != "") {
    msop_input_.reset(new lslidar_driver::InputPCAP(pnh, UDP_PORT_NUMBER,
...
int LslidarDriver::getScan(std::vector<ScanPoint> &points, ros::Time &scan_time,
                           double &scan_duration) // 获取数据和相对时间
{
...
bool LslidarDriver::initialize() // 根据连接方法初始化
{
  ...
  if (pubPointCloud2)
    pointcloud_pub =
        nh.advertise<sensor_msgs::PointCloud2>(pointcloud_topic, 10);
  ROS_INFO("Initialised lslidar without error");
  return true;
}

void LslidarDriver::recvThread_crc(int &count,
                                   int &link_time) // 获取串口数据时检测是否超时
{
  if (count <= 0)
    link_time++;
  else
...
int LslidarDriver::receive_data(unsigned char *packet_bytes) // 获取串口数据
{
  ...
  if (lidar_name == "M10" || lidar_name == "M10_DOUBLE" ||
      lidar_name == "M10_GPS" || lidar_name == "M10_P" ||
      lidar_name == "M10_PLUS") {
  ...
  if (lidar_name == "N10" || lidar_name == "L10" || lidar_name == "N10_P") {
    if (packet_bytes[PACKET_SIZE - 1] !=
        N10_CalCRC8(packet_bytes, PACKET_SIZE - 1))
      return 0;
  }
  return len;
}

uint8_t LslidarDriver::N10_CalCRC8(unsigned char *p,
                                   int len) // N10雷达的数据包校验
{
  int sum = 0;
  for (int i = 0; i < len; i++)
...
void LslidarDriver::difop_processing(
    unsigned char *packet_bytes) // 处理设备包的数据
{
                                    ...
                                    int len) // 处理每一包的数据
{
  ...
  if (lidar_name == "M10_GPS" || lidar_name == "M10") {
    ...
    if ((err_data_84 * 256 + err_data_85) == 0xFFFF ||
        packet_bytes[86] >= 0xF5) {
  ...
  if (use_gps_ts) {
    ...
    sub_second =
        (packet_bytes[PACKET_SIZE - 6] * 256 + packet_bytes[PACKET_SIZE - 5]) *
            1000000 +
        (packet_bytes[PACKET_SIZE - 4] * 256 + packet_bytes[PACKET_SIZE - 3]) *
            1000;
  ...
  for (int num = 0; num < package_points; num++) {
    ...
    if ((s * 256 + z) != 0xFFFF) {
      ...
      } else if ((lidar_name == "M10_P" || lidar_name == "M10_PLUS") &&
                 !high_reflection) {
        scan_points_[idx].range = double(s * 256 + (z)) / 1000.f;
      ...
      if ((degree + (degree_interval / invalidValue * num)) > 360)
        scan_points_[idx].degree =
            degree + (degree_interval / invalidValue * num) - 360;
      else
        scan_points_[idx].degree =
            degree + (degree_interval / invalidValue * num);
    } else
      continue;
    if ((scan_points_[idx].degree < last_degree &&
         scan_points_[idx].degree < 5 && last_degree > 355) ||
        idx >= points_size_) {
    ...
    if (((scan_points_[idx].degree < last_degree &&
      ...
      for (int k = 0; k < count_num; k++) {
        ...
        if (scan_points_[k].range < min_range ||
            scan_points_[k].range > max_range)
          scan_points_[k].range = 0;
        if (scan_points_[k + 3000].range < min_range ||
            scan_points_[k + 3000].range > max_range)
          scan_points_[k + 3000].range = 0;
      }
      boost::unique_lock<boost::mutex> lock(mutex_);
      scan_points_bak_.resize(scan_points_.size());
  ...
  while (ros::ok()) {
    ...
    if (lidar_name == "N10_P" || lidar_name == "M10_DOUBLE") {
      if (pubScan) {
        ...
        if (use_gps_ts) {
          msg.header.stamp =
              ros::Time(sweep_end_time_gps, sweep_end_time_hardware);
        } else {
        ...
        for (int i = 0; i < count_num; i++) {
          ...
          if (n10p_double_echo == false) {
            if (msg.intensities[point_idx] <= 5 &&
                msg.intensities[point_idx] > 0) {
              msg.ranges[point_idx] = std::numeric_limits<float>::infinity();
              msg.intensities[point_idx] = 0;
            }
      ...
      if (pubPointCloud2) {
        ...
        for (uint16_t i = 0; i < count_num; i++) {
          ...
          if (!pass_point) {
            ...
            point.timestamp = timestamp - point_idx * (scan_time / count_num);
            point.x =
                points[i + 3000].range * cos(M_PI / 180 * points[i].degree);
            point.y =
                -points[i + 3000].range * sin(M_PI / 180 * points[i].degree);
            point.z = 0;
      ...
      if (pubScan) {
        ...
        for (int i = 0; i < count_num; i++) {
          ...
          if (lidar_name == "M10" || lidar_name == "M10_P" ||
              lidar_name == "M10_PLUS") {
  ...
  if (interface_selection == "net") {
    ...
    while (true) {
      ...
      if ((lidar_name == "M10" || lidar_name == "M10_DOUBLE" ||
           ...
           lidar_name == "M10_PLUS") &&
          compensation) {
        if (packet->data[2] == 0x55 && packet->data[3] == 0x00 &&
            packet->data[186] == 0xFA && packet->data[187] == 0xFB) {
          len = 188;
          difop = true;
        }
      }
      if (len <= 0 || len >= 1000 || packet->data[0] != 0xa5 ||
          packet->data[1] != 0x5a)
        continue;
      for (int i = 0; i < len; i++) {
