...
bool check_AutoCharge_data = false; // 自动回充数据校验位
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "wheeltec_robot"); // ROS initializes and sets the node
                                           // name //ROS初始化 并设置节点名称
...
float turn_on_robot::Odom_Trans(uint8_t Data_High, uint8_t Data_Low) {
  ...
  short transition_16;
  transition_16 = 0;
  transition_16 |= Data_High
                   << 8; // Get the high 8 bits of data   //获取数据的高8位
  transition_16 |= Data_Low; // Get the lowest 8 bits of data //获取数据的低8位
  data_return =
      (transition_16 / 1000) +
      (transition_16 % 1000) * 0.001; // The speed unit is changed from mm/s to
                                      // m/s //速度单位从mm/s转换为m/s
  return data_return;
}

// 设置自动回充状态服务
bool turn_on_robot::Set_Charge_Callback(turtlesim::Spawn::Request &req,
  ...
  if (round(req.x) == 1)
    ...
    Send_Data.tx[1] = 0,
    AutoRecharge = 0; // 取消自动回充时,同时取消cmd_vel里的标志位状态

  Send_Data.tx[2] = 0; // 是否使能设置
  Send_Data.tx[3] = 0;
  Send_Data.tx[4] = 0;
  Send_Data.tx[5] = 0;
  Send_Data.tx[6] = 0;
  ...
  Send_Data.tx[9] = Check_Sum(
      9, SEND_DATA_CHECK); // For the BCC check bits, see the Check_Sum function
                           // //BCC校验位，规则参见Check_Sum函数
  Send_Data.tx[10] = FRAME_TAIL; // frame tail 0x7D //帧尾0X7D
  try {
    Stm32_Serial.write(
        Send_Data.tx,
        sizeof(Send_Data.tx)); // Sends data to the downloader via serial port
                               // //通过串口向下位机发送数据
  ...
  if (Send_Data.tx[1] == 0) {
    if (charge_set_state ==
        0) // 等待底盘回传的回充状态再判断是否调用服务成功,形成闭环
      AutoRecharge = 0, res.name = "true";
    else
      res.name = "false";
  } else {
    if (charge_set_state ==
        1) // 等待底盘回传的回充状态再判断是否调用服务成功,形成闭环
      res.name = "true";
    else
      res.name = "false";
  }
...
void turn_on_robot::Cmd_Vel_Callback(const geometry_msgs::Twist &twist_aux) {
  ...
  try {
    ...
    ROS_ERROR_STREAM(
        "Unable to send data through serial port"); // If sending data fails, an
...
/**************************************
Date: March 1, 2022
Function: Infrared connection speed topic subscription Callback function,
according to the subscription command through the serial port to set the
infrared connection speed 功能:
红外对接速度话题订阅回调函数Callback，根据订阅的指令通过串口发指令设置红外对接速度
***************************************/
void turn_on_robot::Red_Vel_Callback(const geometry_msgs::Twist &twist_aux) {
  ...
  Send_Data.tx[1] = 3; // Infrared docking speed setting flag bit = 3
                       // //红外对接速度设置标志位=3
  Send_Data.tx[2] = 0; // set aside //预留位
...
Function: Subscription robot recharge flag bit topic, used to tell the lower
machine speed command is normal command or recharge command 功能:
订阅机器人是否回充标志位话题，用于告诉下位机速度命令是正常命令还是回充命令
***************************************/
void turn_on_robot::Recharge_Flag_Callback(
    const std_msgs::Int8 &Recharge_Flag) {
...
void turn_on_robot::Publish_ImuSensor() {
  sensor_msgs::Imu
      Imu_Data_Pub; // Instantiate IMU topic data //实例化IMU话题数据
  Imu_Data_Pub.header.stamp = ros::Time::now();
  Imu_Data_Pub.header.frame_id =
      gyro_frame_id; // IMU corresponds to TF coordinates, which is required to
                     // use the robot_pose_ekf feature pack
                     // IMU对应TF坐标，使用robot_pose_ekf功能包需要设置此项
  Imu_Data_Pub.orientation.x =
      Mpu6050.orientation.x; // A quaternion represents a three-axis attitude
                             // //四元数表达三轴姿态
  ...
  Imu_Data_Pub.orientation_covariance[0] =
      1e6; // Three-axis attitude covariance matrix //三轴姿态协方差矩阵
  Imu_Data_Pub.orientation_covariance[4] = 1e6;
  Imu_Data_Pub.orientation_covariance[8] = 1e-6;
  Imu_Data_Pub.angular_velocity.x =
      Mpu6050.angular_velocity.x; // Triaxial angular velocity //三轴角速度
  Imu_Data_Pub.angular_velocity.y = Mpu6050.angular_velocity.y;
  Imu_Data_Pub.angular_velocity.z = Mpu6050.angular_velocity.z;
  Imu_Data_Pub.angular_velocity_covariance[0] =
      1e6; // Triaxial angular velocity covariance matrix //三轴角速度协方差矩阵
  Imu_Data_Pub.angular_velocity_covariance[4] = 1e6;
  Imu_Data_Pub.angular_velocity_covariance[8] = 1e-6;
  Imu_Data_Pub.linear_acceleration.x =
      Mpu6050.linear_acceleration.x; // Triaxial acceleration //三轴线性加速度
...
void turn_on_robot::Publish_Odom() {
  ...
  nav_msgs::Odometry
      odom; // Instance the odometer topic data //实例化里程计话题数据
  odom.header.stamp = ros::Time::now();
  odom.header.frame_id =
      odom_frame_id; // Odometer TF parent coordinates //里程计TF父坐标
  odom.pose.pose.position.x = Robot_Pos.X; // Position //位置
  odom.pose.pose.position.y = Robot_Pos.Y;
  odom.pose.pose.position.z = Robot_Pos.Z;
  odom.pose.pose.orientation =
      odom_quat; // Posture, Quaternion converted by Z-axis rotation
                 // //姿态，通过Z轴转角转换的四元数

  odom.child_frame_id =
      robot_frame_id; // Odometer TF subcoordinates //里程计TF子坐标
  odom.twist.twist.linear.x =
      Robot_Vel.X; // Speed in the X direction //X方向速度
  odom.twist.twist.linear.y =
      Robot_Vel.Y; // Speed in the Y direction //Y方向速度
  odom.twist.twist.angular.z =
      Robot_Vel.Z; // Angular velocity around the Z axis //绕Z轴角速度

  // There are two types of this matrix, which are used when the robot is at
  // rest and when it is moving.Extended Kalman Filtering officially provides 2
  // matrices for the robot_pose_ekf feature pack
  ...
  if (Robot_Vel.X == 0 && Robot_Vel.Y == 0 && Robot_Vel.Z == 0)
    ...
    // relatively small, and the data of the encoder will be considered more
    // reliable 如果velocity是零，说明编码器的误差会比较小，认为编码器数据更可靠
    ...
    // that may be brought by the encoder in motion, the data of IMU is
...
void turn_on_robot::Publish_Voltage() {
  std_msgs::Float32
      voltage_msgs; // Define the data type of the power supply voltage
                    // publishing topic //定义电源电压发布话题的数据类型
  ...
  if (Count_Voltage_Pub++ > 10) {
    ...
    voltage_msgs.data = Power_voltage; // The power supply voltage is obtained
                                       // //电源供电的电压获取
    voltage_publisher.publish(
        voltage_msgs); // Post the power supply voltage topic unit: V, volt
                       // //发布电源电压话题单位：V、伏特
  }

  if (((Power_voltage < 20 && robot_type == "Plus") ||
       (Power_voltage < 10 && robot_type == "Mini")) &&
      Charging == 0 && time_period >= 2) {
    cout << RED << "Robot battery: " << Power_voltage
         << " is too low. Need charging." << endl
         << RESET;
...
Function: Pub the topic whether the robot finds the infrared signal (charging
station) 功能: 发布机器人是否寻找到红外信号(充电桩)的话题
***************************************/
...
void turn_on_robot::Publish_Charging() {
  ...
  if (last_charging == false && Charging == true)
    cout << GREEN << "Robot is charging." << endl << RESET;
  if (last_charging == true && Charging == false)
    cout << RED << "Robot charging has disconnected." << endl << RESET;
...
to byte n-2 are compared with byte n-1, which is a BCC check Input parameter:
Count_Number: Check the first few bytes of the packet 功能:
串口通讯校验函数，数据包n有个字节，第n-1个字节为校验位，第n个字节位帧尾。第1个字节到第n-2个字节数据按位异或的结果与第n-1个字节对比，即为BCC校验
输入参数： Count_Number：数据包前几个字节加入校验
...
// 自动回充专用校验位
unsigned char turn_on_robot::Check_Sum_AutoCharge(unsigned char Count_Number,
                                                  unsigned char mode) {
  unsigned char check_sum = 0, k;
  if (mode == 0) // Receive data mode //接收数据模式
  {
    for (k = 0; k < Count_Number; k++) {
      check_sum = check_sum ^
                  Receive_AutoCharge_Data.rx[k]; // By bit or by bit //按位异或
    }
  }
...
computer, and then the data is converted to international units Update Note:
This checking method can lead to read error data or correct data not to be
processed. Instead of this checking method, frame-by-frame checking is now used.
...
bool turn_on_robot::Get_Sensor_Data() {
  short transition_16 = 0, j = 0, Header_Pos = 0,
        Tail_Pos = 0; // Intermediate variable //中间变量
  static int flag_error = 0,
             temp = 1; // Static variable that records the error flag and
                       // location //静态变量，用于记录出错标志位和出错位置
  uint8_t Receive_Data_Pr[RECEIVE_DATA_SIZE] = {0},
          Receive_Data_Tr[temp] = {
              0}; // Temporary variable to save the data of the lower machine
                  // //临时变量，保存下位机数据
  if (flag_error == 0) // Normal condition detected //检测到正常情况
    Stm32_Serial.read(
        Receive_Data_Pr,
        sizeof(Receive_Data_Pr)); // Read the data sent by the lower computer
                                  // through the serial port
                                  // //通过串口读取下位机发送过来的数据
  else if (flag_error == 1) // Error condition detected 检测到错误情况
  {
    // Read wrong bit data through serial port, read only and do not process, so
    // that the correct data is read next time
  ...
  /*//View the received raw data directly and debug it for
  use//直接查看接收到的原始数据，调试使用
  ...
  */
  ...
  if (Tail_Pos == (Header_Pos + 23)) {
    // If the end of the frame is the last bit of the packet, copy the packet
    // directly to receive_data.rx
    // 如果帧尾在数据包最后一位，直接复制数据包到Receive_Data.rx
    //  ROS_INFO("1-----");
    memcpy(Receive_Data.rx, Receive_Data_Pr, sizeof(Receive_Data_Pr));
    flag_error = 0; // Error flag position 0 for next reading
                    // //错误标志位置0，便于下次读取
  } else if (Header_Pos == (1 + Tail_Pos)) {
    // If the header is behind the end of the frame, record the position of the
    // header so that the next reading of the error bit data can correct the
    // data position
    // 如果帧头在帧尾后面，记录帧头出现的位置，便于下次读取出错位数据以纠正数据位置
    //|********7D (7B************|**********7D) 7B************|
    //  ROS_INFO("2-----");
    temp = Header_Pos; // Record the length of the next read, calculated to be
                       // exactly the position of the frame head
                       // //记录下一次读取的长度，经计算正好为帧头的位置
    flag_error = 1; // Error flag position 1, error bit array for next read
                    // //错误标志位置1，让下一次读取出错位数组
    return false;
  } else {
    ////其它情况则认为数据包有错误，这种情况一般是正常的数据，但是除帧头帧尾在数据中间出现了7B或7D的数据
    // In other cases, the packet is considered to be faulty
    // This is generally normal data, but there is 7B or 7D data in the middle
    // of the data except for the frame header and end. ROS_INFO("3-----");
    return false;
  }
  ...
  */
  ...
  Receive_Data.Frame_Header =
      Receive_Data.rx[0]; // The first part of the data is the frame header 0X7B
                          // //数据的第一位是帧头0X7B
  Receive_Data.Frame_Tail =
      Receive_Data.rx[23]; // The last bit of data is frame tail 0X7D
                           // //数据的最后一位是帧尾0X7D

  if (Receive_Data.Frame_Header ==
      FRAME_HEADER) // Judge the frame header //判断帧头
  {
    if (Receive_Data.Frame_Tail ==
        FRAME_TAIL) // Judge the end of the frame //判断帧尾
    {
      if (Receive_Data.rx[22] ==
          Check_Sum(
              22,
              READ_DATA_CHECK)) // BCC check passes or two packets are
                                // interlaced //BCC校验通过或者两组数据包交错
      {
        Receive_Data.Flag_Stop = Receive_Data.rx[1]; // set aside //预留位
        Robot_Vel.X = Odom_Trans(
            Receive_Data.rx[2],
            Receive_Data.rx[3]); // Get the speed of the moving chassis in the X
                                 // direction //获取运动底盘X方向速度
        Robot_Vel.Y = Odom_Trans(
            Receive_Data.rx[4],
            Receive_Data.rx
                [5]); // Get the speed of the moving chassis in the Y direction,
                      // The Y speed is only valid in the omnidirectional mobile
                      // robot chassis
                      // 获取运动底盘Y方向速度，Y速度仅在全向移动机器人底盘有效
        Robot_Vel.Z = Odom_Trans(
            Receive_Data.rx[6],
            Receive_Data.rx[7]); // Get the speed of the moving chassis in the Z
                                 // direction //获取运动底盘Z方向速度

        // MPU6050 stands for IMU only and does not refer to a specific model.
        // It can be either MPU6050 or MPU9250
        // Mpu6050仅代表IMU，不指代特定型号，既可以是MPU6050也可以是MPU9250
        Mpu6050_Data.accele_x_data =
            IMU_Trans(Receive_Data.rx[8],
                      Receive_Data.rx[9]); // Get the X-axis acceleration of the
                                           // IMU     //获取IMU的X轴加速度
        Mpu6050_Data.accele_y_data =
            IMU_Trans(Receive_Data.rx[10],
                      Receive_Data.rx[11]); // Get the Y-axis acceleration of
                                            // the IMU     //获取IMU的Y轴加速度
        Mpu6050_Data.accele_z_data =
            IMU_Trans(Receive_Data.rx[12],
                      Receive_Data.rx[13]); // Get the Z-axis acceleration of
                                            // the IMU     //获取IMU的Z轴加速度
        Mpu6050_Data.gyros_x_data =
            IMU_Trans(Receive_Data.rx[14],
                      Receive_Data.rx[15]); // Get the X-axis angular velocity
                                            // of the IMU //获取IMU的X轴角速度
        Mpu6050_Data.gyros_y_data =
            IMU_Trans(Receive_Data.rx[16],
                      Receive_Data.rx[17]); // Get the Y-axis angular velocity
                                            // of the IMU //获取IMU的Y轴角速度
        Mpu6050_Data.gyros_z_data =
            IMU_Trans(Receive_Data.rx[18],
                      Receive_Data.rx[19]); // Get the Z-axis angular velocity
                                            // of the IMU //获取IMU的Z轴角速度
        // Linear acceleration unit conversion is related to the range of IMU
        // initialization of STM32, where the range is ±2g=19.6m/s^2
        ...
        // The gyroscope unit conversion is related to the range of STM32's IMU
        // when initialized. Here, the range of IMU's gyroscope is ±500°/s
        // Because the robot generally has a slow Z-axis speed, reducing the
        // range can improve the accuracy
        ...
        Power_voltage = transition_16 / 1000 +
                        (transition_16 % 1000) *
                            0.001; // Unit conversion millivolt(mv)->volt(v)
                                   // //单位转换毫伏(mv)->伏(v)
...
/**************************************
Date: November 18, 2021
Function: Read and verify the data sent by the lower computer frame by frame
through the serial port, and then convert the data into international units
...
bool turn_on_robot::Get_Sensor_Data_New() {
  ...
  uint8_t i = 0, check = 0, check2 = 0, error = 1, error2 = 1,
          Receive_Data_Pr[1]; // Temporary variable to save the data of the
                              // lower machine //临时变量，保存下位机数据
  ...
  */
  ...
  Receive_Data.rx[count] =
      Receive_Data_Pr[0]; // Fill the array with serial data //串口数据填入数组
  ...
  if ((Receive_Data_Pr[0] == FRAME_HEADER) ||
      count > 0) // Ensure that the first data in the array is FRAME_HEADER
  ...
  if (count2 == AutoCharge_DATA_SIZE) {
    count2 = 0;
    if (Receive_AutoCharge_Data.rx[AutoCharge_DATA_SIZE - 1] ==
        AutoCharge_TAIL) // 确认帧尾
    {
      check2 = Check_Sum_AutoCharge(6, 0); // 校验位计算
      if (check2 ==
          Receive_AutoCharge_Data.rx[AutoCharge_DATA_SIZE - 2]) // 校验正确
      {
        error2 = 0;
      }
      if (error2 == 0) // 校验正确开始赋值
      {
        ...
        Charging_Current =
            transition_16 / 1000 + (transition_16 % 1000) * 0.001; // 充电电流
  ...
  if (count == 24) // Verify the length of the packet //验证数据包的长度
  {
    ...
    {
      check = Check_Sum(
          22, READ_DATA_CHECK); // BCC check passes or two packets are
                                // interlaced //BCC校验通过或者两组数据包交错
...
void turn_on_robot::Control() {
  ...
  while (ros::ok()) {
    Sampling_Time =
        (_Now - _Last_Time)
            .toSec(); // Retrieves time interval, which is used to integrate
                      // velocity to obtain displacement (mileage)
                      // 获取时间间隔，用于积分速度获得位移(里程)
    _Now = ros::Time::now();
    if (true ==
        Get_Sensor_Data_New()) // The serial port reads and verifies the data
                               // sent by the lower computer, and then the data
                               // is converted to international units
                               // 通过串口读取并校验下位机发送过来的数据，然后数据转换为国际单位
    {
      ...
      Robot_Pos.X +=
          ...
          Sampling_Time; // Calculate the displacement in the X direction, unit:
                         // m //计算X方向的位移，单位：m
      Robot_Pos.Y +=
          (Robot_Vel.X * sin(Robot_Pos.Z) + Robot_Vel.Y * cos(Robot_Pos.Z)) *
          Sampling_Time; // Calculate the displacement in the Y direction, unit:
                         // m //计算Y方向的位移，单位：m
      Robot_Pos.Z += Robot_Vel.Z *
                     Sampling_Time; // The angular displacement about the Z
                                    // axis, in rad //绕Z轴的角位移，单位：rad

      // Calculate the three-axis attitude from the IMU with the angular
      // velocity around the three-axis and the three-axis acceleration
      ...
      Quaternion_Solution(
          ...
          Mpu6050.linear_acceleration.y, Mpu6050.linear_acceleration.z);

      Publish_Odom();      // Pub the speedometer topic //发布里程计话题
      Publish_ImuSensor(); // Pub the IMU topic //发布IMU话题
      Publish_Voltage();   // Pub the topic of power supply voltage
                         // //发布电源电压话题
      _Last_Time = _Now; // Record the time and use it to calculate the time
                         // interval //记录时间，用于计算时间间隔
    }

    // 自动回充数据话题
    if (check_AutoCharge_data) {
      Publish_Charging(); // Pub a topic about whether the robot is charging
                          // //发布机器人是否在充电的话题
      Publish_RED(); // Pub the topic whether the robot finds the infrared
                     // signal (charging station)
                     // //发布机器人是否寻找到红外信号(充电桩)的话题
      Publish_ChargingCurrent(); // Pub the charging current topic
                                 // //发布充电电流话题
...
turn_on_robot::turn_on_robot() : Sampling_Time(0), Power_voltage(0) {
  ...
  // The private_nh.param() entry parameter corresponds to the initial value of
  // the name of the parameter variable on the parameter server
  // private_nh.param()入口参数分别对应：参数服务器上的名称  参数变量名  初始值
  private_nh.param<std::string>(
      "usart_port_name", usart_port_name,
      "/dev/wheeltec_controller"); // Fixed serial port number //固定串口号
  private_nh.param<int>("serial_baud_rate", serial_baud_rate,
                        115200); // Communicate baud rate 115200 to the lower
                                 // machine //和下位机通信波特率115200
  private_nh.param<std::string>(
      "odom_frame_id", odom_frame_id,
      "odom_combined"); // The odometer topic corresponds to the parent TF
                        // coordinate //里程计话题对应父TF坐标
  private_nh.param<std::string>(
      "robot_frame_id", robot_frame_id,
      "base_footprint"); // The odometer topic corresponds to sub-TF coordinates
                         // //里程计话题对应子TF坐标
  private_nh.param<std::string>("gyro_frame_id", gyro_frame_id,
                                "gyro_link"); // IMU topics correspond to TF
                                              // coordinates //IMU话题对应TF坐标
  private_nh.param<std::string>("car_mode", robot_type,
                                "mini_mec"); // 机器人型号
  robot_type = robot_type.substr(0, 4);
  if (robot_type != "mini")
    robot_type = "Plus";
  else
    robot_type = "Mini";
  ...
  voltage_publisher = n.advertise<std_msgs::Float32>(
      "PowerVoltage",
      10); // Create a battery-voltage topic publisher //创建电池电压话题发布者
  odom_publisher = n.advertise<nav_msgs::Odometry>(
      "odom", 50); // Create the odometer topic publisher //创建里程计话题发布者
  imu_publisher = n.advertise<sensor_msgs::Imu>(
      "imu", 20); // Create an IMU topic publisher //创建IMU话题发布者
  Charging_publisher = n.advertise<std_msgs::Bool>(
      "robot_charging_flag", 20); // Create robot charging flag position topic
                                  // publisher //创建机器人充电标志位话题发布者
  Charging_current_publisher = n.advertise<std_msgs::Float32>(
      "robot_charging_current", 20); // Create the charging current topic
                                     // publisher //创建充电电流话题发布者
  RED_publisher = n.advertise<std_msgs::UInt8>(
      "robot_red_flag",
      20); // Create a topic publisher that finds the infrared signal flag bit
           // //创建寻找到红外信号标志位话题发布者
  SetCharge_Service = n.advertiseService(
      "set_charge", &turn_on_robot::Set_Charge_Callback, this);

  // Set the velocity control command callback function
  // 速度控制命令订阅回调函数设置
  Cmd_Vel_Sub =
      n.subscribe("cmd_vel", 100, &turn_on_robot::Cmd_Vel_Callback, this);
  // Ir docking speed topic subscription callback function
  // 红外对接速度话题订阅回调函数
  Red_Vel_Sub =
      n.subscribe("red_vel", 100, &turn_on_robot::Red_Vel_Callback, this);
  // Automatic recharge mode flag bit subscription callback function
  // 自动回充模式标志位订阅回调函数
  Recharge_Flag_Sub = n.subscribe("robot_recharge_flag", 100,
                                  &turn_on_robot::Recharge_Flag_Callback, this);
  ...
  try {
    ...
    Stm32_Serial.setPort(usart_port_name); // Select the serial port number to
                                           // enable //选择要开启的串口号
    Stm32_Serial.setBaudrate(serial_baud_rate); // Set the baud rate
                                                // //设置波特率
    serial::Timeout _time =
        serial::Timeout::simpleTimeout(2000); // Timeout //超时等待
    Stm32_Serial.setTimeout(_time);
    Stm32_Serial.open(); // Open the serial port //开启串口
  } catch (serial::IOException &e) {
    ROS_ERROR_STREAM(
        "wheeltec_robot can not open serial port,Please check the serial port "
        "cable! "); // If opening the serial port fails, an error message is
                    // printed //如果开启串口失败，打印错误信息
  }
  if (Stm32_Serial.isOpen()) {
    ROS_INFO_STREAM(
        "wheeltec_robot serial port opened"); // Serial port opened successfully
                                              // //串口开启成功提示
  }
}
/**************************************
...
ends its life cycle 功能:
析构函数，只执行一次，当对象结束其生命周期时系统会调用这个函数
***************************************/
turn_on_robot::~turn_on_robot() {
  // Sends the stop motion command to the lower machine before the turn_on_robot
  // object ends 对象turn_on_robot结束前向下位机发送停止运动命令
  ...
  Send_Data.tx[7] = 0;
  Send_Data.tx[9] =
      Check_Sum(9, SEND_DATA_CHECK); // Check the bits for the Check_Sum
                                     // function //校验位，规则参见Check_Sum函数
  Send_Data.tx[10] = FRAME_TAIL;
  try {
    Stm32_Serial.write(
        Send_Data.tx,
        sizeof(Send_Data.tx)); // Send data to the serial port //向串口发数据
  } catch (serial::IOException &e) {
    ROS_ERROR_STREAM(
        "Unable to send data through serial port"); // If sending data fails, an
                                                    // error message is printed
