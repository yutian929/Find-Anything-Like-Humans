...
class Image_converter:
    def __init__(self):  # 类成员初始化函数
        ...
        )  # 初始化订阅者,rospy.Publisher()功能是创建订阅者类并输出
        self.image_pub = rospy.Publisher(
            "/usb_cam/image_raw/compressed", CompressedImage, queue_size=10
        )  # 初始化发布者,rospy.Publisher()功能是创建发布者类并输出。queue_size为队列长度，当消息发布后订阅者暂时没有接收处理，则该消息进入缓存循环发送，当队列满后最老的数据被踢出队列
...
if (
    __name__ == "__main__"
):  # 这段判断的作用是，如果本py文件是直接运行的则判断通过执行if内的内容，如果是import到其他的py文件中被调用(模块重用)则判断不通过
