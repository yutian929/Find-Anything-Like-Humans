...
wheeltec_joy::wheeltec_joy() {
  ...
  pub = n.advertise<geometry_msgs::Twist>("cmd_vel",
                                          1); // 将速度发给机器人底盘节点
  sub = n.subscribe<sensor_msgs::Joy>("joy", 10, &wheeltec_joy::callback,
                                      this); // 订阅手柄发来的数据
}

void wheeltec_joy::callback(
    const sensor_msgs::Joy::ConstPtr &Joy) // 键值回调函数
{
  ...
  if (vangle_key > 0)
    vlinear_z.data = vangular;
  else if (vangle_key < 0)
    vlinear_z.data = -vangular;
  else
    vlinear_z.data = 0;
  // 处理数据
