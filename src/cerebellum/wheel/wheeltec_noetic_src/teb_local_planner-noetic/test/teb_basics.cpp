...
TEST(TEBBasic, autoResizeLargeValueAtEnd) {
  ...
  teb.addPoseAndTimeDiff(teb_local_planner::PoseSE2(10., 0., 0.),
                         dt + 2 * dt_hysteresis);

  // auto resize + test of the result
  teb.autoResize(dt, dt_hysteresis, 3, 100, false);
  for (int i = 0; i < teb.sizeTimeDiffs(); ++i) {
    ASSERT_LE(teb.TimeDiff(i), dt + dt_hysteresis + 1e-3)
        << "dt is greater than allowed: " << i;
    ASSERT_LE(dt - dt_hysteresis - 1e-3, teb.TimeDiff(i))
...
TEST(TEBBasic, autoResize) {
  ...
  // modify the timediff in the middle and add a pose with a smaller timediff as
  // the last one
