...
from avsm.srv import Guide, GuideRequest, Query, QueryRequest


class AVSMTester:
    def __init__(self):
        rospy.init_node("avsm_tester")

        # 等待服务初始化
        rospy.loginfo("Waiting for Query and Guide service...")
        rospy.wait_for_service("/avsm_manager/query")
        self.query_client = rospy.ServiceProxy("/avsm_manager/query", Query)
        rospy.wait_for_service("/avsm_manager/guide")
        self.guide_client = rospy.ServiceProxy("/avsm_manager/guide", Guide)
        ...
        rospy.loginfo("AVSM Tester initialized complete.")
    ...
    def go_to_target(self, target):
        ...
        rospy.loginfo(f"查询结果: {query_res}")
        # query服务直接获取poses
        avsm_poses = query_res.avsm_poses
        if not avsm_poses:
            rospy.logwarn("查询结果为空，无法导航")
            return
        # 依次尝试导航到每个可能的目标点
        for idx, nav_pose in enumerate(avsm_poses):
            rospy.loginfo(f"尝试导航点 {idx+1}/{len(avsm_poses)} : {nav_pose}")
        ...
        rospy.logwarn("所有导航点均失败，无法到达目标位置")
